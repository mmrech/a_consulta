...
└─────────────────┘  │
              ...
              │  Gemini API  │
              └──────────────┘
Security Issues: - ❌ API key embedded in JavaScript bundle - ❌ Visible via browser DevTools - ❌ No server-side rate limiting - ❌ No request authentication - ❌ Vulnerable to abuse and quota exhaustion
...
└─────────────────┘      │ Auth required    │      └──────────────┘
                         └──────────────────┘
Security Benefits: - ✅ API key never exposed to client - ✅ Server-side authentication and authorization - ✅ Centralized rate limiting - ✅ Request validation and sanitization - ✅ Comprehensive logging and monitoring - ✅ Compliance with security best practices
 
Implementation Steps
Phase 1: Backend API Implementation (Python FastAPI)
Files to Create/Modify:
1.	backend/app/routes/ai_service.py (NEW)
 	"""
AI Service Routes
...
2.	backend/app/services/gemini_client.py (NEW)
 	"""
Gemini API Client
...
class GeminiClient:
    ...
    async def generate_pico(self, pdf_text: str):
        ...
        pass
3.	backend/.env (UPDATE - NEVER COMMIT)
 	# Backend environment (SECURE - never committed to git)
GEMINI_API_KEY=your_actual_api_key_here
4.	backend/requirements.txt (UPDATE)
 	google-genai>=1.0.0
Phase 2: Frontend Migration
Files to Modify:
1.	src/services/AIService.ts - Replace direct Gemini calls with BackendClient
BEFORE:
...
1.	Remove API key loading (lines 53-78)
 	// DELETE these lines after migration:
const API_KEY = import.meta.env.VITE_GEMINI_API_KEY || ...
let ai: GoogleGenAI | null = null;
function initializeAI() { ... }
2.	Update all 7 AI functions:
3.	generatePICO() → /api/ai/generate-pico
4.	generateSummary() → /api/ai/generate-summary
5.	validateFieldWithAI() → /api/ai/validate-field
6.	findMetadata() → /api/ai/find-metadata
7.	handleExtractTables() → /api/ai/extract-tables
8.	handleImageAnalysis() → /api/ai/analyze-image
9.	handleDeepAnalysis() → /api/ai/deep-analysis
Phase 3: Environment Configuration
Files to Update:
1.	.env.example (REMOVE frontend API key)
 	# DELETE (moved to backend):
# VITE_GEMINI_API_KEY=...

# ADD (backend URL):
VITE_BACKEND_URL=http://localhost:8000
2.	backend/.env.example (ADD secure key storage)
...
1. Backend API	8-12 hours	FastAPI knowledge, Python
2. Frontend Migration	6-8 hours	TypeScript, BackendClient
3. Environment Config	2-3 hours	DevOps, deployment knowledge
4. Testing	4-6 hours	Pytest, Jest
5. Deployment	2-4 hours	Infrastructure access
Total	22-33 hours	3-5 days for 1 developer
 
Current Status
Implementation Status: ❌ Not Started
Blockers: - None - ready to start when prioritized
Alternative Solutions: - Use Firebase App Check (easier but less secure) - Use serverless functions (AWS Lambda, Vercel Functions) - Accept risk with strict API key restrictions (not recommended for production)
